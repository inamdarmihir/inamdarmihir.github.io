  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-7 menubar-blur bg-black/70 flex items-center justify-between px-4 text-white/90 text-[13px] select-none">
      ...
      <div className="flex items-center gap-3 sm:gap-5">
        ...
        <span className="hidden lg:inline text-white/70">History</span>
        <span className="hidden lg:inline text-white/70">Bookmarks</span>
        <span className="hidden xl:inline text-white/70">Window</span>
        <span className="hidden xl:inline text-white/70">Help</span>
      </div>

      {/* Right: system tray */}
      <div className="flex items-center gap-2 sm:gap-3">
        <Volume2 size={13} className="hidden sm:block text-white/80" />
        <Wifi size={13} className="text-white/80" />
        <Battery size={14} className="text-white/80" />
        <Search size={13} className="hidden sm:block text-white/80" />
        <span className="hidden sm:block text-white/80">{date}</span>
