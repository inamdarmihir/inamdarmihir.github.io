...
export default function Terminal({ onClose }: TerminalProps) {
  ...
  return (
    ...
    >
      {/* Title bar */}
      <div
        className="flex items-center gap-2 px-3 py-2.5 bg-[#2a2a2e] cursor-grab active:cursor-grabbing select-none"
      ...
      <div className="bg-[#12121a] p-4 h-72 overflow-y-auto font-mono text-[12px] leading-relaxed">
          ...
          <div key={i}>
            {line.type === 'cmd' && (
              <div className="flex items-start gap-1">
                ...
                <span className="text-white/40 select-none flex-shrink-0">$</span>
                <span className="text-[#cdd6f4] ml-1 break-all">{line.text}</span>
              </div>
            )}
            {line.type === 'out' && (
              <div className="pl-0 break-all" style={{ color: line.color || '#cdd6f4' }}>{line.text}</div>
            )}
            {line.type === 'comment' && (
        ...
        {isTyping && (
          <div className="flex items-start gap-1">
            ...
            <span className="text-[#cba6f7] select-none flex-shrink-0">~</span>
            <span className="text-white/40 select-none flex-shrink-0">$</span>
            <span className="text-[#cdd6f4] ml-1">{currentText}</span>
